...
class mesos_vagrant(ShutItModule):
	...
	def build(self, shutit):
		...
		for c in ('git',):
			if not shutit.command_available(c):
				shutit.install(c)
		shutit.send('cd')
		shutit.send('vagrant plugin install vagrant-ombnibus')
		shutit.send('vagrant plugin install vagrant-berkshelf')
		shutit.send('vagrant plugin install vagrant-hosts')
...
def module():
	return mesos_vagrant(
		'shutit.mesos_vagrant.mesos_vagrant.mesos_vagrant', 1308628950.01,
		description='',
		maintainer='',
